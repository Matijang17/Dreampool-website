...
import { getStaticBlogPost, type StaticBlogPost } from '@/lib/staticBlogPosts'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  let post = null
  try { post = await client.fetch(blogPostQuery, { slug }) } catch {}
  if (!post) {
    const sp = getStaticBlogPost(slug)
    if (sp) {
      return {
        title: sp.metaTitle || sp.title,
        description: sp.metaDescription || sp.excerpt,
        ...
        openGraph: {
          ...
          url: `/blog/${slug}`,
          images: sp.image ? [{ url: sp.image, width: 1200, height: 630 }] : [],
          publishedTime: sp.publishedAt,
          authors: sp.author ? [sp.author] : undefined,
        },
        twitter: {
...
export default async function BlogPostPage({ params }: Props) {
  ...
  if (!post) {
    const sp = getStaticBlogPost(slug)
    if (sp) return <StaticBlogPostPage slug={slug} post={sp} />
    return (
...
function StaticBlogPostPage({ slug, post }: { slug: string; post: StaticBlogPost }) {
  const date = new Date(post.publishedAt).toLocaleDateString('sl-SI', { day: 'numeric', month: 'long', year: 'numeric' })
    ...
    <>
      ...
      <article className="min-h-screen bg-pool-navy pt-24">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          {post.category && (
            <div className="flex gap-2 mb-4">
              <span className="text-pool-accent text-xs uppercase tracking-widest bg-pool-accent/10 px-3 py-1 rounded-sm">
                {post.category}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 text-pool-white/40 text-sm mb-10 pb-10 border-b border-pool-white/10">
            {post.author && <span>Avtor: {post.author}</span>}
            <span>·</span>
            <span>{date}</span>
          </div>

          {/* Body */}
          <div className="prose-pool">{post.body}</div>

          {/* FAQ */}
          {post.faqs.length > 0 && (
            <section className="mt-16 pt-10 border-t border-pool-white/10">
              <h2 className="font-display text-3xl font-bold text-pool-white mb-8">Pogosta vprašanja</h2>
              <div className="space-y-6">
                {post.faqs.map((faq) => (
                  <div key={faq.q}>
                    <h3 className="font-display text-lg font-bold text-pool-white mb-2">{faq.q}</h3>
                    <p className="text-pool-white/60 leading-relaxed">{faq.a}</p>
                  </div>
                ))}
              </div>
            </section>
